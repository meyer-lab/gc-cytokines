...
def catplot_comparison(ax, df, legend=False, Mut=True):
    ...
    # plot predicted EC50
    sns.pointplot(x="Cell Type", y="EC-50", hue="IL",
                  data=df.loc[(df['Time Point'] == 60.) & (df["Data Type"] == 'Predicted')],
                  legend=legend, legend_out=legend, ax=ax, marker='^', join=False, scale=0.5)

    # plot experimental EC50
    sns.pointplot(x="Cell Type", y="EC-50", hue="IL",
                  data=df.loc[(df['Time Point'] == 60.) & (df["Data Type"] == 'Experimental')],
                  legend=False, legend_out=False, ax=ax, marker='o', join=False, scale=0.5)
