...
def commonTraf(trafficking=True):
    ...
    kfwd = pm.Lognormal("kfwd", mu=np.log(0.0001), sigma=0.5, shape=1)

    if trafficking:
        endo = pm.Lognormal("endo", mu=np.log(0.01), sigma=0.1, shape=1)
        activeEndo = pm.Lognormal("activeEndo", mu=np.log(0.1), sigma=0.1, shape=1)
