...
def runCkineSP(tps, rxntfr, actV, preT=0.0, prestim=None):
    ...
    sensV = np.delete(sensV, [0, 6], 1)
    sensV = np.delete(sensV, [1, 3], 1)
    sensV = np.delete(sensV, [3, 7], 1)
    sensV = np.delete(sensV, [7], 1)
    sensV = np.delete(sensV, [8], 1)
    sensV = np.delete(sensV, [9], 1)
    sensV = np.delete(sensV, [10], 1)
    sensV = np.delete(sensV, [11, 31], 1)
    print(sensV)
    print(type(sensV))
    print(sensV.shape)
