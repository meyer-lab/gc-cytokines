...
We used CVODE to numerically solve our stiff system of ODEs [@hindmarsh2005sundials]. The inputs for our solver were the initial values of each ligand and receptor-complex, time, and all rate parameters. To determine the initial values of each receptor on the cell surface, we ran the model for a long time with no cytokine present which allowed the system to reach steady state. A tolerance of $1.5 * 10^{-6}$ was used in solving our ODEs.
...
Here, $X_{r}$ is the reconstructed tensor and $X$ is the original tensor. By iterating over different possible R component numbers, the percent explained variance is obtained.
