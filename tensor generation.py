from ckine.model import solveAutocrine, fullModel, getTotalActiveCytokine, __active_species_IDX, printModel
import numpy as np
...
#print (mat.shape[0]) gives 1024 for the above values; Need to update
...
tfR = np.ones(11) * (5* 10**-2)#I am also supposed to know these values
...
for ii in range(len(mat)):
    ...
    if d['message'] == "Integration successful.":
        y_of_combos[ii] = temp[ii]# only assign values to ys if there isn't an error message; all errors will still be 0
